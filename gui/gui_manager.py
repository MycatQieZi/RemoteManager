...
class GUIManager():
    def __init__(self, **fns):
        ...
        for key, thread in self.threads.items():
            sys_tray_fn_kwargs[key] = lambda: self.threaded_execution_wrapper(
                fns[key], thread, f"{key}Thread")
        self.sysTray = SysTray(self.app, **sys_tray_fn_kwargs)


    def threaded_execution_wrapper(self, fn_action, thread_instance, thread_name=None):
